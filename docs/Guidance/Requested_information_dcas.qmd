...
aliases:
  - /Requested_information_dcas.html
categories: [DCAS, data availability, code availability, data, code, citations, examples]
---
...
This site provides a [template README](../Templates/template-README.qmd) that incorporates the DCAS into the README, as is the style for many journals in economics.

### Examples

#### Example for public use data included in data archive:

> The paper uses data obtained from IPUMS (Ruggles et al, 2017). IPUMS-CPS does not currently provide the ability to store or reference custom extracts, but allows for redistribution of a subset of the data to meet journal requirements for accessing data related to a particular publication (https://cps.ipums.org/cps/terms.shtml). The archive contains the extracted data, codebook in the folder "data/IPUMS". The data citation in the main article has the full URL.
...
In some cases, governments have a list of their (named) registers. For instance, Statistics Denmark provides the full list of registers at [http://www.dst.dk/extranet/forskningvariabellister/Oversigt%20over%20registre.html](http://www.dst.dk/extranet/forskningvariabellister/Oversigt%20over%20registre.html). These can be used to craft data citations (see [Data citation guidance](../Guidance/Data_citation_guide.qmd)). Data availability statements should describe how each such register can be accessed:
...
### Example for primary data, fully included in archive:

"The data used in this paper were collected by the authors via [an online survey / a laboratory experiment / field interviews] conducted in [YEAR]. The dataset is included in the replication archive in the folder data/raw/. Documentation of the survey instrument and sampling procedure is provided in data/raw/codebook.pdf [and below]. The data are made available under a Creative Commons Attribution 4.0 International (CC BY 4.0) license."

### Example for primary data, restricted due to confidentiality

“The data used in this paper were collected by the authors via [an online survey / a laboratory experiment / field interviews] conducted in [YEAR] under participant consent and ethics restrictions. Public release of record-level data is not permitted. A de-identified synthetic dataset, codebook, and full replication code are available at https://doi.org/10.XXXX/XXXX; qualified researchers may request secure access to the original data via [application link/contact].”

## Additional examples

Further examples can be found

- [for commercial data](../Samples/DCAS_Commercial_data.qmd)
- [for various restricted data](../Samples/DCAS_Restricted_data.qmd)